...
class Dashboard extends Component {
  ...
  makePayment = (seller_address, amount, land_id) => async () => {
    ...
    alert(amount);
    await this.state.FarmInstance.methods.payment(
      seller_address,
      land_id
    ).send({
  ...
  componentDidMount = async () => {
    ...
    try {
      ...
      var registered = await this.state.FarmInstance.methods.isBuyer(currentAddress).call();
      ...
      for (var i = 0; i < count; i++) {
        var paid = await this.state.FarmInstance.methods.isPaid(i + 1).call();
        var price = await this.state.FarmInstance.methods.getPrice(i + 1).call();
